...
import { maxReactionCount, tankaReactions } from '@/const/tanka';
...
export const POST = async (req: NextRequest) => {
  try {
    ...
    // Bad Request
    if (tanka_id == null) {
      return new NextResponse('Bad Request: tanka_id is empty.', { status: 400 });
    }
    if (!reaction) {
      return new NextResponse('Bad Request: reaction is empty.', { status: 400 });
    }
    if (!tankaReactions.includes(reaction)) {
      return new NextResponse('Bad Request: reaction is not defined.', { status: 400 });
    }
